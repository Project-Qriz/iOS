...
public final class CustomTextField: UITextField {
    ...
    private lazy var passwordToggleButton: UIButton = {
        let button = UIButton(type: .system)
        button.setImage(UIImage.eyeSlashIcon, for: .normal)
        button.tintColor = .coolNeutral700
        button.addAction(
            UIAction { [weak self] _ in
                guard let self = self else { return }
                self.isSecureTextEntry.toggle()
                let newImage = self.isSecureTextEntry ? UIImage.eyeSlashIcon : UIImage.eyeIcon
    ...
    public func updateTimerLabel(_ remainingTime: Int) {
        let minutes = remainingTime / 60
        let seconds = remainingTime % 60
        timerLabel.text = String(format: "%02d:%02d", minutes, seconds)
    }

    public func updateRightView(_ type: RightViewType) {
        setupRightView(type)
    }
