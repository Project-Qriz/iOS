...
public struct SurveyCheckList {
    ...
    public static func getChapter(_ idx: Int) -> Chapter {
        switch idx {
        case 0...4:
            return .dataModeling
        case 5...9:
    ...
    public static func getConceptItem(_ idx: Int) -> ConceptItem {
        let chapter = self.getChapter(idx)
        let conceptItems = chapter.conceptItems
        let conceptItem = conceptItems.first { $0.title == list[idx] }
        if let conceptItem {
            return conceptItem
        } else {
            print("getConceptItem Method Received wrong argv")
            return ConceptItem(title: "", fileName: "")
        }
    }
}
