//
//  SignUpService.swift
//  QRIZ
//
//  Created by 김세훈 on 3/15/25.
//

import Foundation

public protocol SignUpService {
    
    /// 이메일 인증번호 전송 및 중복 확인
    func sendEmail(_ email: String) async throws -> EmailSendResponse
    
    /// 인증번호 인증
    func EmailAuthentication(email: String, authNumber: String) async throws -> EmailAuthenticationResponse
    
    /// 아이디 중복 체크
    func checkUsernameDuplication(username: String) async throws -> UsernameDuplicationResponse
    
    /// 회원가입
    func join(
        username: String,
        password: String,
        nickname: String,
        email: String
    ) async throws -> JoinResponse
}

public final class SignUpServiceImpl: SignUpService {
    
    // MARK: - Properties
    
    private let network: Network
    
    // MARK: - Initialize
    
    public init(network: Network = NetworkImpl(session: URLSession.shared)) {
        self.network = network
    }
    
    // MARK: - Functions
    
    public func sendEmail(_ email: String) async throws -> EmailSendResponse {
        let request = EmailSendRequest(email: email)
        return try await network.send(request)
    }
    
    public func EmailAuthentication(email: String, authNumber: String) async throws -> EmailAuthenticationResponse {
        let request = EmailAuthenticationRequest(email: email, authNumber: authNumber)
        return try await network.send(request)
    }
    
    public func checkUsernameDuplication(username: String) async throws -> UsernameDuplicationResponse {
        let request = UsernameDuplicationRequest(username: username)
        return try await network.send(request)
    }
    
    public func join(
        username: String,
        password: String,
        nickname: String,
        email: String
    ) async throws -> JoinResponse {
        let request = JoinRequest(
            username: username,
            password: password,
            nickname: nickname,
            email: email
        )
        return try await network.send(request)
    }
}
